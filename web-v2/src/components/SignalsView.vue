...
const data = reactive({signals:null, scanned:0, total_signals:0, predict_is_rank:false})
...
const columns = computed(() => {
  ...
  const base = [
    ...
    { title:'预测5d', key:'predict_5d', width:70, align:'right', render(r){ const v=r.predict_5d; if(v==null) return '—'; if(data.predict_is_rank) return h('span',{style:{color:'var(--c-text-dim)',fontSize:'11px'}},'P'+(v*100).toFixed(0)); return h('span',{style:{color:v>=0?'#ef4444':'#10b981',fontSize:'11px'}},(v>=0?'+':'')+(v*100).toFixed(1)+'%') },
  ]
  if (isNarrow.value) return base
  return [
    ...base,
    { title:'预测10d', key:'predict_10d', width:70, align:'right', render(r){ const v=r.predict_10d; if(v==null) return '—'; if(data.predict_is_rank) return h('span',{style:{color:'var(--c-text-dim)',fontSize:'11px'}},'P'+(v*100).toFixed(0)); return h('span',{style:{color:v>=0?'#ef4444':'#10b981',fontSize:'11px'}},(v>=0?'+':'')+(v*100).toFixed(1)+'%') },
    { title:'预测20d', key:'predict_20d', width:70, align:'right', render(r){ const v=r.predict_20d; if(v==null) return '—'; if(data.predict_is_rank) return h('span',{style:{color:'var(--c-text-dim)',fontSize:'11px'}},'P'+(v*100).toFixed(0)); return h('span',{style:{color:v>=0?'#ef4444':'#10b981',fontSize:'11px'}},(v>=0?'+':'')+(v*100).toFixed(1)+'%') },
