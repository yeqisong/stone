    ...
    try:
        ...
        if cache_row:
            ...
            data_tables = [
                ...
                {'label':'ETF日K线','rows':q("SELECT COUNT(*) FROM daily_quote dq WHERE EXISTS (SELECT 1 FROM stock_master sm WHERE sm.stock_code=dq.stock_code AND sm.stock_type='etf')"),'items':q("SELECT COUNT(*) FROM stock_master WHERE stock_type='etf'")},
