...
| Naive UI + 主题系统 | §14 — 前端模型页 |

---

## 14. 前端设计 — 模型页

> 🔗 复用 K道 `web-v2/` 基础设施：Naive UI 组件库、暗色/浅色主题、DAG-WS 实时推送、DagView 流程图。

### 14.1 导航位置

在 K道顶部 Tab 栏新增 **🧠 模型** 标签（字母 `m` 已被选股占用，使用 `a` 或 `ai`）：

```
💼 持仓  📊 选股  🔴 信号  📋 个股  📊 状态  🧠 模型  ⚙️ 设置
```

### 14.2 页面总布局

模型页采用**左侧列表 + 右侧详情**双栏布局：

```
┌──────────────────────────────────────────────────────────┐
│  🧠 模型管理                              [+ 创建新版本]   │
├────────────────────┬─────────────────────────────────────┤
│  模型版本列表       │  详情区（根据选中 Tab 切换）          │
│                    │                                      │
│  ┌──────────────┐  │  [基本信息] [训练] [评估] [实盘] [指标] │
│  │ v2.1 ACTIVE  │  │  ───────────────────────────────     │
│  │ 布林+MACD    │  │                                      │
...
**左侧列表**：每行显示版本号、状态标签、策略名、核心指标（夏普）、创建时间。点击切换右侧详情。

**右侧 5 个 Tab**：

| Tab | 内容 | 数据来源 |
|-----|------|---------|
| 基本信息 | 四层配置只读展示、状态机当前节点、操作按钮（创建/训练/审批/归档） | `GET /api/v1/models/{version}` |
| 训练 | Optuna 进度（已完成的 trial 数/总数、最佳 score 曲线）、参数重要性柱状图、DAG 流程图（训练节点状态） | `GET /api/v1/models/{version}/trials` + DAG-WS |
| 评估 | 核心指标卡片 + 权益曲线 + Walk-Forward 明细 + 版本对比 | §11 设计 |
| 实盘 | 信号胜率趋势、forward 收益分布、健康度 5 维度雷达图、最近信号列表 | §12 设计 + `GET /api/v1/models/{version}/health` |
| 指标 | 指标数据管理（见 §14.6） | — |
...
│  🧠 模型版本                    [+ 创建] │
│  ────────────────────────────────── │
│  ● v2.1  ACTIVE   布林+MACD+RSI     │
│    夏普 2.15  胜率 58%  06-10 上线   │
│  ────────────────────────────────── │
│  ◐ v2.0  ARCHIVED  布林+MACD        │
│    夏普 1.82  胜率 52%  05-20 归档   │
│  ────────────────────────────────── │
│  ○ v1.3  PENDING   布林+MACD+量能   │
│    夏普 2.31  胜率 61%  待审批       │
│  ────────────────────────────────── │
│  ○ v1.2  DRAFT     (训练中...)      │
│  ────────────────────────────────── │
...
**状态标签**：`DRAFT` 🔵 / `TRAINING` 🟡 / `PENDING` 🟠 / `ACTIVE` 🟢 / `ARCHIVED` ⚫ / `REJECTED` 🔴

**操作按钮**（根据状态显示）：
- DRAFT → [开始训练]
- PENDING → [审批] [拒绝]
- ACTIVE → [查看实盘]
- 任意状态 → [归档]（仅 ACTIVE 可归档）
...
│  ✚ 创建模型版本                           │
│  ────────────────────────────────────────│
│  版本号      [v_____._____]  (自动建议)    │
│  模型名称    [________________]            │
│                                            │
│  ── 数据配置 ──                            │
│  股票池      [全部A股 ▼]  [自定义清单]       │
│  训练区间    从 [2021-01-01] 到 [2025-12-31]│
│  测试区间    从 [2026-01-01] 到 [2026-06-12]│
│                                            │
│  ── 特征配置 ──                            │
│  ☑ BOLL(20,2)  ☑ MACD(12,26,9)            │
│  ☑ RSI(14)     ☑ ATR(14)                  │
│  ☑ MA(5,20,60) ☑ 量能                      │
│                                            │
│  ── 模型配置 ──                            │
...
**创建流程**：
1. 填写配置 → 点击"创建并检查数据"
2. 后端校验：训练区间内 K 线数据是否充足、指标表是否覆盖该区间
3. 数据不足 → 提示"请先生成指标数据"（跳转到指标管理）
4. 数据充足 → INSERT `model_versions`（status=DRAFT）→ 返回版本号

### 14.5 训练 Tab

选中 DRAFT 版本 → 右侧切换到"训练"Tab：
...
**实时更新**：训练进度通过 DAG-WS 推送，Optimization 曲线每完成一个 trial 更新一次。

**训练完成** → 状态自动变为 VALIDATING → 验证完成后变为 PENDING → 飞书推送审批卡片。

### 14.6 指标管理 Tab

选中任意版本（或全局入口）→ 右侧切换到"指标"Tab：
...
│  ────────────────────────────────────────│
│                                            │
│  最近更新: 2026-06-13 15:35  (增量)         │
│                                            │
│  ┌─ 指标表状态 ────────────────────────┐   │
│  │  表名              行数      最新日期  │   │
│  │  boll        5,850×1,250  2026-06-12 │   │
│  │  macd        5,850×1,250  2026-06-12 │   │
│  │  rsi         5,850×1,250  2026-06-12 │   │
│  │  atr         5,850×1,250  2026-06-12 │   │
│  │  ma          5,850×1,250  2026-06-12 │   │
│  │  volume      5,850×1,250  2026-06-12 │   │
│  └────────────────────────────────────┘   │
│                                            │
│  操作:                                     │
│  ┌─────────────────────────────────────┐  │
...
**触发方式**：调用 `/api/dag_trigger`：

| 操作 | body |
|------|------|
| 全量初始化全部 | `{node: "indicator_full", date: "2026-06-13"}` |
| 增量更新今日 | `{node: "indicator_incr", date: "2026-06-13"}` |
| 单独全量 BOLL | `{node: "indicator_full", date: "2026-06-13", indicator: "boll"}` |
| 单独增量 BOLL | `{node: "indicator_incr", date: "2026-06-13", indicator: "boll"}` |

> 全量初始化耗时较长（~30 分钟），通过 DAG-WS 实时推送进度到状态页 DagView。

### 14.7 状态页 DAG 集成

在 K道状态页的 DagView 流程图中增加模型相关节点：

```
原有: cron → daily_update → kline → fund → treemap/strategy → stats → daily_completeness

扩展（在 kline 和 treemap/strategy 之间插入）:
  ... → kline ─┬→ indicator_full ─────────────────────┐
                ├→ indicator_incr → model_signal → model_health
                └→ fund (原有) → treemap/strategy → ...
```

**节点依赖关系**：
- `indicator_full` / `indicator_incr` 依赖 `kline`（需要当日 K 线数据）
- `model_signal` 依赖 `indicator_incr`（需要当日指标数据）
- `model_health` 依赖 `model_signal`（需要当日信号数据）
- `indicator_full` 为手动触发，不参与每日自动流水线
- `indicator_incr` / `model_signal` / `model_health` 参与每日自动流水线

**DagView 渲染**：新增节点按 K道现有配色渲染（default→pending→running→success/failed），连线自动推导状态。

### 14.8 前端新增文件清单

| 文件 | 内容 |
|------|------|
| `web-v2/src/components/ModelView.vue` | 模型页主组件（左侧列表 + 右侧 Tab） |
| `web-v2/src/components/ModelCreate.vue` | 创建/编辑模型版本抽屉 |
| `web-v2/src/components/ModelTraining.vue` | 训练进度 + Optuna 图表 |
| `web-v2/src/components/ModelEval.vue` | 评估仪表盘（复用 §11 设计） |
| `web-v2/src/components/ModelLive.vue` | 实盘监控面板（复用 §12 设计） |
| `web-v2/src/components/ModelIndicators.vue` | 指标管理面板 |
| `web-v2/src/stores/model.js` | 模型 Pinia store |
| `web-v2/src/App.vue` | 新增 Tab "🧠 模型" (字母 `a`) |
